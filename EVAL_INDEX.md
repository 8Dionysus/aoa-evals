...
| aoa-code-observation-refactor-integrity | capability | draft | Checks whether a provider-neutral code-observation envelope stays coherent across refactor-sensitive lineage, freshness, invalidation, provenance, parity, and affected-test cases without claiming provider correctness. |
...
| aoa-runtime-latency-tradeoff | comparative | draft | Checks whether a runtime comparison preserves a bounded latency and resource-use tradeoff claim without overreading it as reasoning quality, agent behavior, or cross-host performance. |
...
| aoa-output-vs-process-gap | comparative | draft | Compares artifact-side and process-side readings on the same bounded cases after both standalone artifact review and standalone workflow review are visible under matched conditions. |
