...
Read repeated-window movement only after:
1. the one-run anchor workflow surface is already clear
2. frozen-baseline comparison has been ruled out or read separately when needed
3. the ordered window sequence and anchor surface have been named

Then read:
4. `aoa-longitudinal-growth-snapshot`
5. `aoa-eval-integrity-check` when wording, routing, or maturity posture risks growth-by-association

## Required comparability disclosures

Each window must keep:
- one anchor workflow surface
- one `context_note` that discloses why the window is still comparable or why comparability is limited
- one `transition_note` that explains why movement relative to the previous window is real, flat, unstable, or regressive

The window sequence stays bounded only when:
- order is explicit
- anchor surface stays constant
