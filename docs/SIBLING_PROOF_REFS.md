...
`docs/SIBLING_PROOF_REFS.md` is the compatibility map for proof references that
...
The canary matrix source is `scripts/sibling_canary_matrix.json`.
...
7. Run `python scripts/run_sibling_canary.py --repo-root . --format json` when
