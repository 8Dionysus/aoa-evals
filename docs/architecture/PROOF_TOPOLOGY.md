...
| Owner callable procedure | `skills/aoa-evals/`, `skills/port.manifest.json`, and the exact `.agents/skills/aoa-evals/` projection | applicability, typed procedure, input/output ABI, effects, failure dispositions, composition, and handoffs for central proof work | receives a bounded proof question or reviewed owner pressure; emits a selection, review, evolution, no-change, or handoff disposition | source eval bundles and admitted evidence keep proof meaning; the projection is discovery-only; shared cross-repo selection and local apply stay with `aoa-skills` |
...
| `skills/` | owner-admitted repo-specific callable procedures | contains one admitted `aoa-evals` bundle with internal select, review, and evolve modes; manual evidence owns admission, the common port owns only structural parity, and shared workflows remain with `aoa-skills` |
| `.agents/` | maintained agent lanes and derived repo discovery | durable agent-facing district; proof canon stays with source proof objects, and `.agents/skills/aoa-evals` is the exact generated projection of the owner home, never proof or procedure source |
