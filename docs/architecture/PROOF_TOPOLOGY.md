...
| MCP access-plane contract | `docs/architecture/AOA_EVALS_MCP_CONTRACT.md` | resources, tools, prompts, owner split, stop-lines, local-port write gates, and candidate-only runtime evidence posture for `aoa_evals` | receives generated readers, runtime-candidate readers, proof questions, and sibling local-port pressure; emits compact proof/candidate read access plus narrow gated sibling-local eval-port writes for the stack-owned MCP implementation | MCP output remains inspection support; source bundles and manifests keep proof authority; sibling repos own local port files; `abyss-stack` owns runtime service code only |
...
| `.agents/` | maintained agent lanes and exported support skills | durable agent-facing district; proof canon stays with source proof objects |
| `.agents/skills/` | installed local skills that support repo work | supports agent operation; proof canon stays with source proof objects |
