...
| MCP access-plane contract | `docs/architecture/AOA_EVALS_MCP_CONTRACT.md` | resources, tools, prompts, owner split, stop-lines, and candidate-only runtime evidence posture for `aoa_evals` | receives generated readers, runtime-candidate readers, and proof questions; emits compact read-only access for stack-owned MCP implementation | MCP output remains inspection support; source bundles and manifests keep proof authority; `abyss-stack` owns runtime service code only |
