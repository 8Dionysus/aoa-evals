...
| Owner callable procedure | `skills/aoa-evals-skills/`, `skills/port.manifest.json`, and the managed user projection selected by the central `os-user-default` profile | applicability, typed procedure, input/output ABI, effects, failure dispositions, composition, and handoffs for central proof work | receives a bounded proof question or reviewed owner pressure; emits a selection, review, evolution, no-change, or handoff disposition | source eval bundles and admitted evidence keep proof meaning; the installed projection is discovery-only; shared cross-repo selection and local apply stay with `aoa-skills` |
| Agent guidance | `AGENTS.md`, local `AGENTS.md` cards, `DESIGN.AGENTS.md`, `.agents/` | editing route, local risk, validation, closeout posture | receives topology and source-owner rules; emits bounded operating guidance | source proof surfaces keep verdict meaning; guidance owns edit route and validation |
...
| `skills/` | owner-admitted repo-specific callable procedures | contains one admitted `aoa-evals-skills` bundle with internal select, review, and evolve modes; manual evidence owns admission, the common port owns only structural parity, and shared workflows remain with `aoa-skills` |
| `.agents/` | maintained agent lanes | durable agent-facing district for execution posture; proof canon stays with source proof objects, and globally installed owner skills are not duplicated here |
