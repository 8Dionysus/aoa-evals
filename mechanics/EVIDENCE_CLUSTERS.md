...
| `examples` | public-safe example compatibility district | route-card-only root posture; no active root examples payload remains here | bundle examples stay in `evals/**/examples/`; audit, receipt, bridge, and domain examples live under owning mechanic parts | root route-card guard plus active route residue checks |
...
| `templates` | eval bundle template compatibility district | route-card-only root posture; no active root template payload remains here | active eval bundle authoring template lives under `mechanics/proof-object/parts/bundle-authoring/templates/` | root route-card guard, proof-object bundle-authoring checks, and root repository validation route |
...
| `proof-object` | `mechanics/proof-object/README.md`, `mechanics/proof-object/parts/bundle-contracts/schemas/eval-manifest.schema.json`, `evals/workflow/aoa-verification-honesty/EVAL.md`, `generated/eval_catalog.json` |
