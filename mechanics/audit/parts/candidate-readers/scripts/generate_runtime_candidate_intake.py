...
TEMPLATE_INDEX_PATH = PART_ROOT / "generated" / "runtime_candidate_template_index.min.json"
OUTPUT_PATH = PART_ROOT / "generated" / "runtime_candidate_intake.min.json"
...
def build_runtime_candidate_intake_payload() -> dict[str, object]:
    ...
    if not isinstance(payload, dict) or not isinstance(payload.get("templates"), list):
        raise SystemExit("[error] mechanics/audit/parts/candidate-readers/generated/runtime_candidate_template_index.min.json must contain a templates list")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != payload:
            raise SystemExit(
                "[error] mechanics/audit/parts/candidate-readers/generated/runtime_candidate_intake.min.json is out of date; "
                "run mechanics/audit/parts/candidate-readers/scripts/generate_runtime_candidate_intake.py"
            )
        print("[ok] mechanics/audit/parts/candidate-readers/generated/runtime_candidate_intake.min.json is current")
        return 0

    write_output(payload)
    print("[ok] wrote mechanics/audit/parts/candidate-readers/generated/runtime_candidate_intake.min.json")
    return 0
