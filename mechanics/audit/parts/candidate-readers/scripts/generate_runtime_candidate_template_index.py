...
RUNTIME_EVIDENCE_SELECTION_EXAMPLES_ROOT = (
    REPO_ROOT / "mechanics" / "audit" / "parts" / "selected-evidence-packets" / "examples"
)
ARTIFACT_VERDICT_HOOK_EXAMPLES_ROOTS = (
    REPO_ROOT / "mechanics" / "audit" / "parts" / "artifact-verdict-hooks" / "examples",
    REPO_ROOT / "mechanics" / "checkpoint" / "parts" / "a2a-summon-return" / "examples",
    REPO_ROOT / "mechanics" / "checkpoint" / "parts" / "restartable-inquiry" / "examples",
    REPO_ROOT / "mechanics" / "checkpoint" / "parts" / "self-agent-posture" / "examples",
)
OUTPUT_PATH = PART_ROOT / "generated" / "runtime_candidate_template_index.min.json"
...
def build_runtime_candidate_template_index_payload() -> dict[str, object]:
    ...
    for path in sorted(RUNTIME_EVIDENCE_SELECTION_EXAMPLES_ROOT.glob("runtime_evidence_selection.*.example.json")):
    ...
    for root in ARTIFACT_VERDICT_HOOK_EXAMPLES_ROOTS:
        for path in sorted(root.glob("artifact_to_verdict_hook.*.example.json")):
            payload = read_json(path)
            if not isinstance(payload, dict):
                raise SystemExit(f"[error] {path.relative_to(REPO_ROOT).as_posix()} must contain an object")
    ...
    return {
        ...
        "source_of_truth": {
            ...
            "artifact_to_verdict_hook_examples": [
                root.relative_to(REPO_ROOT).as_posix() + "/artifact_to_verdict_hook.*.example.json"
                for root in ARTIFACT_VERDICT_HOOK_EXAMPLES_ROOTS
            ],
        },
        "templates": entries,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != payload:
            raise SystemExit(
                "[error] mechanics/audit/parts/candidate-readers/generated/runtime_candidate_template_index.min.json is out of date; "
                "run mechanics/audit/parts/candidate-readers/scripts/generate_runtime_candidate_template_index.py"
            )
        print("[ok] mechanics/audit/parts/candidate-readers/generated/runtime_candidate_template_index.min.json is current")
        return 0

    write_output(payload)
    print("[ok] wrote mechanics/audit/parts/candidate-readers/generated/runtime_candidate_template_index.min.json")
    return 0
