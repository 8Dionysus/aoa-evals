...
It is a `boundary-bridge` part surface, not an `orchestrator` parent mechanic.
The operation here is proof-anchor alignment across owner boundaries: local
eval obligations point at `aoa-agents` orchestrator classes, playbook families,
and memo surfaces while keeping each stronger owner in charge of its own
meaning.
