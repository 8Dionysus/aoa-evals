...
PARTS_ROOT = PART_ROOT.parent
SCHEMA_ROOTS = (
    PART_ROOT / "schemas",
    PARTS_ROOT / "certification-gate" / "schemas",
)
ESCAPE_VALUE = "__wave5_not_allowed__"

WAVE5_CONTRACTS = (
    ('boundary_guard_verdict_v1', 'boundary_guard_verdict_v1.json'),
    ('governed_release_verdict_v1', 'governed_release_verdict_v1.json'),
    ('handoff_integrity_verdict_v1', 'handoff_integrity_verdict_v1.json'),
    ('installation_smoke_verdict_v1', 'installation_smoke_verdict_v1.json'),
    ('multi_office_regression_verdict_v1', 'multi_office_regression_verdict_v1.json'),
    ('office_scope_fidelity_verdict_v1', 'office_scope_fidelity_verdict_v1.json'),
    ('office_train_smoke_verdict_v1', 'office_train_smoke_verdict_v1.json'),
    ('replay_audit_verdict_v1', 'replay_audit_verdict_v1.json'),
    ('rollback_drill_verdict_v1', 'rollback_drill_verdict_v1.json'),
    ('train_release_certification_verdict_v1', 'train_release_certification_verdict_v1.json'),
)


def load_contract(stem: str, schema_file: str) -> tuple[dict[str, object], dict[str, object]]:
    schema_path = next((root / schema_file for root in SCHEMA_ROOTS if (root / schema_file).exists()), None)
...
class ExperienceWave5SeedContractTests(unittest.TestCase):
    ...
    def test_experience_wave5_examples_match_schemas(self) -> None:
        ...
        for stem, schema_file in WAVE5_CONTRACTS:
            ...
            example_path = PART_ROOT / "examples" / f"{stem}.example.json"
            if schema_path is None:
                missing_pairs.append(f"{example_path.relative_to(PART_ROOT)} -> {schema_file}")
                continue
