...
MECHANIC_ROOT = Path(__file__).resolve().parents[3]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Append bounded eval-result receipts to the owner-local live JSONL log."
    )
    parser.add_argument(
        "--input",
        action="append",
    ...
    parser.add_argument(
        ...
        help="Owner-local JSONL log that should receive newly published eval receipts.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[ok] appended {appended} eval receipts to {log_path}")
