...
This card applies to the canonical `aoa-evals/skills/` home.
...
- Keep `select`, `review`, and `evolve` as internal modes until held-out manual
  work proves that a split adds independent trigger, ABI, composition, and
  outcome value.
- Keep cross-repository selection, exact local application, and local no-fit
  proposal with shared `aoa-eval`; begin this owner skill only at central proof
  meaning or an `aoa-evals` owner decision.
- Do not make techniques a required dependency of the owner skill. A technique
  cited by an eval remains evidence or lineage of that source bundle.
- Do not keep candidates, raw trials, task-local DAGs, temporary rubrics,
  session notes, runtime state, owner facts, tests, or generated proof readers
  in this home.
