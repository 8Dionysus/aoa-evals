...
def test_mechanic_legacy_readmes_validate_entry_contract() -> None:
    legacy_readmes = set(mechanic_legacy_validator.MECHANIC_LEGACY_README_FILES)

    assert not any(
        issue.location in legacy_readmes
        for issue in mechanic_parents_validator.validate_mechanics_parent_allowlist(
            REPO_ROOT
        )
    )
...
def test_mechanic_legacy_archive_route_language_validates_current_routes() -> None:
    assert (
        mechanic_legacy_validator.validate_mechanic_legacy_archive_route_language(
            REPO_ROOT
        )
        == []
    )
...
def test_mechanic_legacy_raw_payload_accounting_validates_current_routes() -> None:
    assert (
        mechanic_legacy_validator.validate_mechanic_legacy_raw_payload_accounting(
            REPO_ROOT
        )
        == []
    )
...
def test_active_legacy_parent_wording_validates_current_routes() -> None:
    assert mechanic_legacy_validator.validate_active_legacy_parent_wording(REPO_ROOT) == []


def test_mechanic_legacy_archive_route_language_rejects_command_blocks(
    tmp_path: Path,
) -> None:
    for path_name in mechanic_legacy_validator.MECHANIC_LEGACY_ARCHIVE_ROUTE_FILES:
    ...
    issues = mechanic_legacy_validator.validate_mechanic_legacy_archive_route_language(
        tmp_path
    )
...
def test_active_legacy_parent_wording_rejects_old_parent_forms(
    ...
    for path_name in mechanic_legacy_validator.ACTIVE_LEGACY_PARENT_WORDING_FORBIDDEN:
        copy_repo_text(tmp_path, path_name)
    for path_name in (
        mechanic_legacy_validator.ACTIVE_LEGACY_PARENT_WORDING_DECISION_NAME,
    ...
    issues = mechanic_legacy_validator.validate_active_legacy_parent_wording(tmp_path)
...
def test_mechanic_legacy_raw_payload_accounting_rejects_unindexed_payload(
    ...
    issues = mechanic_legacy_validator.validate_mechanic_legacy_raw_payload_accounting(
        tmp_path
    )
...
def test_audit_legacy_index_rejects_missing_runtime_evidence_lineage(
    ...
    copy_repo_text(tmp_path, audit_validator.AUDIT_LEGACY_INDEX_NAME)
    index_path = tmp_path / audit_validator.AUDIT_LEGACY_INDEX_NAME
    ...
    assert any(
        issue.location == audit_validator.AUDIT_LEGACY_INDEX_NAME
...
def test_publication_receipts_legacy_index_rejects_missing_root_guide_lineage(
    ...
    copy_repo_text(tmp_path, publication_receipts_validator.PUBLICATION_RECEIPTS_LEGACY_INDEX_NAME)
    index_path = tmp_path / publication_receipts_validator.PUBLICATION_RECEIPTS_LEGACY_INDEX_NAME
    index_path.write_text(
        index_path.read_text(encoding="utf-8").replace(
    ...
    assert any(
        issue.location == publication_receipts_validator.PUBLICATION_RECEIPTS_LEGACY_INDEX_NAME
