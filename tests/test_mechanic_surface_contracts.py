...
def mechanics_issues(repo_root: Path = REPO_ROOT) -> list[validate_repo.ValidationIssue]:
    return validate_repo.validate_mechanics_surfaces(repo_root)
...
def test_proof_loop_smoke_report_rejects_missing_receipt_boundary(
    ...
    copy_repo_text(tmp_path, validate_repo.PROOF_LOOP_SMOKE_REPORT_NAME)
    ...
    assert any(
        issue.location == validate_repo.PROOF_LOOP_SMOKE_REPORT_NAME
...
@pytest.mark.parametrize(
    ...
    (
        ("mechanics/recurrence/", validate_repo.RECURRENCE_MECHANIC_DECISION_NAME),
        ("mechanics/checkpoint/", validate_repo.CHECKPOINT_MECHANIC_DECISION_NAME),
        ("mechanics/experience/", validate_repo.EXPERIENCE_MECHANIC_DECISION_NAME),
        ("mechanics/distillation/", validate_repo.DISTILLATION_MECHANIC_DECISION_NAME),
    ),
)
...
def test_titan_seed_boundary_part_readme_validates_current_contract() -> None:
    assert_no_mechanics_issue_for_location(validate_repo.TITAN_SEED_BOUNDARY_PART_README_NAME)
...
def test_titan_parts_index_readme_keeps_canary_as_payload_form() -> None:
    assert_no_mechanics_issue_for_location(validate_repo.TITAN_PARTS_INDEX_README_NAME)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            "agon",
            {path_name for path_name, _tokens in validate_repo.AGON_PART_README_CONTRACTS},
        ),
        (
            "boundary_bridge",
            {
...
@pytest.mark.parametrize(
    "readme_name",
    (
        validate_repo.PROOF_LOOP_ROUTE_SMOKE_PART_README_NAME,
        validate_repo.GROWTH_CYCLE_DIAGNOSIS_GATE_PART_README_NAME,
        validate_repo.RECURRENCE_CONTROL_PLANE_PART_README_NAME,
        validate_repo.RPG_PROGRESS_UNLOCKS_PART_README_NAME,
    ),
)
